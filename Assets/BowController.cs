...
public class BowController : MonoBehaviour
{
    ...
    [SerializeField] GameObject aim; // 조준선 게임 오브젝트
    ...
    float distance; // 활 시위를 당긴 거리를 저장하기 위한 변수
    GameObject nowArrow; // 현재 발사할 화살
    private void Start()
    {
        aim.SetActive(false); // 조준선 비활성화
    ...
    public void SetRightHandPos()
    {
        ...
        stringSound.Play(); // 화살을 당기는 소리 재생
    ...
    IEnumerator SetRightHand()
    {
        ...
        while (distance < 0.6f)
        {
            ...
            arrowModel.transform.localPosition -= new Vector3(0, 0.1f, 0);
            nowRightHand = transform.InverseTransformPoint
                (new Vector3(arrowModel.transform.position.x,
                             arrowModel.transform.position.y,
                             arrowModel.transform.position.z)); // 월드 좌표를 로컬로 변환
